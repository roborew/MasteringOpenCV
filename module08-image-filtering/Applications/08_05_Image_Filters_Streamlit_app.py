...
if uploaded_file is not None:
    ...
    if option == "None":
        ...
        color = "GRAY"
    elif option == "Combined Effects":
        # Add sliders for controlling the effect strengths
        vignette_level = st.slider("Vignette Strength", 0, 5, 2)
        sketch_blur = st.slider("Sketch Detail", 1, 11, 5, step=2)
        
        # Apply filters in sequence
        output = img.copy()
        output = sepia(output)  # Apply sepia first
        output = vignette(output, vignette_level)  # Add vignette
        # Convert to pencil sketch and blend with sepia
        sketch = pencil_sketch(img, sketch_blur)
        # Blend sketch with previous effects
        output = cv2.addWeighted(output, 0.6, cv2.cvtColor(sketch, cv2.COLOR_GRAY2BGR), 0.4, 0)
